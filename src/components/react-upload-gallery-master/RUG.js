...
import { bytesToSize, getImageDimensions, isAccepted } from '../../utils/UploadGalleryUtils'
        ...
        <div className={`rug ${className}`} style={style}>
          {header &&
            (typeof header === 'function'
              ? header(options)
              : Handle(options, header, this.props.isHidden))}
          ...
          {footer &&
            (typeof footer === 'function'
              ? footer(options)
              : Handle(options, footer, this.props.isHidden))}
          ...
          <input
            ...
            onChange={(event) => {
              if (!this.props.isHidden({ module: { key: 'ImageGallery', value: 'U' } })) {
                this.uploadFiles(event.target.files)
              } else {
                toast(this.props.t('uploadPermissionNotGrantedToast'), 'error')
              }
            }}
          />
        </div>
